...
sub onPrivateMessage {
    ...
    open my $logFH, '>>', 'logs/aiChat_respostas.txt'
        or warning "[aiChat] Falha ao abrir logs/aiChat_respostas.txt: $!\n", "plugin";
    print $logFH "[" . getFormattedDate(time) . "] (From: $sender) : $message\n";
    close $logFH;
    ...
    if ($response) {
        ...
        Misc::chatLog("pm", "(To: $sender) : $response\n") if $config{'logPrivateChat'};

        open my $logFH, '>>', 'logs/aiChat_respostas.txt' 
            or warning "[aiChat] Não foi possível abrir logs/aiChat_respostas.txt: $!\n", "plugin";
        print $logFH "[" . getFormattedDate(time) . "] (To: $sender) : $response\n";
        close $logFH;
    } else {
